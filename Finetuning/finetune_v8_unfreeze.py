...
PRETRAINED = BASE_DIR / "checkpoints_v5" / "best_stgcn_v7_freeze.pth"
...
def parse_args():
    ...
    p.add_argument("--lr",           type=float, default=1e-5) # Cực kỳ nhỏ cho 2 block cuối
    p.add_argument("--lr-head",      type=float, default=1e-4) # Vừa phải cho Head đã học xong
...
def main():
    ...
    if not args.no_wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"stgcn-v8-unfreeze",
                config=vars(args),
                tags=["stgcn", "unfreeze", "transfer_learning"],
            )
    ...
    if PRETRAINED.exists():
        print(f"[Pretrained] Loading V7 weights from {PRETRAINED}")
    ...
    ckpt_path = SAVE_DIR / "best_stgcn_v8_unfreeze.pth"
    ...
    # Save JSON
    json_path = SAVE_DIR / "best_metrics_v8_unfreeze.json"
