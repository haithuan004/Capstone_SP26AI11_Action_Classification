...
finetune_v2.py
==============
Finetune ST-GCN trên dataset đã augment & balance.
- Data source : stgcn_augmented/ (180 train tracks, balanced 45/class)
- Loss        : FocalLoss (gamma=2) + Label Smoothing 0.2
- MixUp       : alpha=0.2
- TTA         : Flip-LR trong evaluation
- Clip length : 100 frames, Stride: 20
- WandB       : project stgcn-augmented
"""
...
# No pretrained weights for SGCN; we train from scratch.
...
class SkeletonDS(Dataset):
    def __init__(self, data, labels, clip_len, is_train=True):
        ...
        self.data     = data[:, :, idx]        # (N, C, clip_len, V, M)
        self.labels   = labels
    ...
    def __getitem__(self, i):
        d = self.data[i].copy()                # (C, T, V, M)
        if self.is_train:
            if random.random() < 0.5: d = aug_flip(d)
            d = aug_noise(d, sigma=random.uniform(0.005, 0.02))
        d = d.squeeze(-1)                      # (C, T, V)
        return torch.from_numpy(d), self.labels[i]
...
def train_epoch(model, loader, criterion, optimizer, device, alpha=0.2, clip_norm=5.0):
    ...
    for data, labels in loader:
        ...
        with torch.cuda.amp.autocast(enabled=(device.type=="cuda")):
            logits, sp_loss = model(data)
            loss   = lam*criterion(logits, ya) + (1-lam)*criterion(logits, yb) + sp_loss
        loss.backward()
...
def parse_args():
    ...
    p.add_argument("--wandb-project",type=str,   default="sgcn-augmented")
    return p.parse_args()
...
def main():
    ...
    if not args.no_wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"sgcn-augmented-clip{args.clip_len}",
                config=vars(args),
                tags=["sgcn", "augmented", "balanced"],
            )
    ...
    # Model
    model = SGCN(in_channels=3, num_classes=args.num_classes, dropout=args.dropout).to(device)
    print("[Pretrained] Not available for SGCN, training from scratch.")
    ...
    ckpt_path = SAVE_DIR / "best_sgcn_augmented.pth"
    ...
    # Save JSON
    json_path = SAVE_DIR / "best_metrics_sgcn_augmented.json"
