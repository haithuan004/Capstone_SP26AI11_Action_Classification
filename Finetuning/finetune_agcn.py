...
def parse_args():
    ...
    p.add_argument("--wandb-project",type=str,   default="agcn-augmented")
    return p.parse_args()
...
def main():
    ...
    if not args.no_wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"agcn-augmented-clip{args.clip_len}",
                config=vars(args),
                tags=["agcn", "augmented", "balanced"],
            )
    ...
    # Model
    model = AGCN(in_channels=3, num_classes=args.num_classes, dropout=args.dropout).to(device)
    print("[Pretrained] Not available for AGCN, training from scratch.")
    ...
    ckpt_path = SAVE_DIR / "best_agcn_augmented.pth"
    ...
    # Save JSON
    json_path = SAVE_DIR / "best_metrics_agcn_augmented.json"
