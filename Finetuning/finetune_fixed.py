"""
finetune_new_data.py
==============
Finetune ST-GCN trên dataset mới (new_data_split).
Dựa trên phương pháp finetune_v2.py:
- Data source : new_data_split/
- Pretrained  : NTU-RGB+D 60
...
DATA_DIR   = BASE_DIR / "kfold_new_data" / "fold_0"
...
def main():
    ...
    if not args.no_wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"stgcn-new-data-clip{args.clip_len}",
                config=vars(args),
                tags=["stgcn", "new_data", "finetune_v2"],
            )
    ...
    if PRETRAINED.exists():
        print(f"[Pretrained] Loading from {PRETRAINED}")
    ...
    ckpt_path = SAVE_DIR / "best_stgcn.pth"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if vl < best_loss:
            ...
            no_improve = 0
            print(f"  *** New best Val Loss={best_loss:.4f} (F1={vf1:.4f}) saved ***")
        else:
            no_improve += 1
    ...
    # Save JSON
    json_path = SAVE_DIR / "best_metrics.json"
