...
def parse_args():
    ...
    p.add_argument("--lr",           type=float, default=0.0) # Không dùng tới vì đã freeze backbone
    p.add_argument("--lr-head",      type=float, default=1e-3) # Tăng LR của Head lên một chút vì chỉ học mỗi Head
...
def main():
    ...
    if not args.no_wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"stgcn-v7-freeze",
                config=vars(args),
                tags=["stgcn", "freeze", "linear_probing"],
            )
    ...
    va_win, va_wlabs = make_windows(va_data, va_labs, args.clip_len, stride=50) # Tăng lượng window validation
    print(f"  Train windows: {len(tr_wlabs)}  Val windows: {len(va_wlabs)}")
    print(f"  Train window dist: {dict(sorted(Counter(tr_wlabs).items()))}")
    ...
    print("[Transfer Learning] Freezing the Backbone...")
    for param in model.parameters():
        param.requires_grad = False
        
    # Mở khóa duy nhất Head phân loại
    ...
    # Optimizer CHỈ nhận các params của Head
    ...
    ckpt_path = SAVE_DIR / "best_stgcn_v7_freeze.pth"
    ...
    # Save JSON
    json_path = SAVE_DIR / "best_metrics_v7_freeze.json"
