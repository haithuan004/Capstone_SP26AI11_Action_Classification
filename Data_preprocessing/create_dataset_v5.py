...
def aug_temporal_scale(data: np.ndarray, scale: float) -> np.ndarray:
    C, T, V, M = data.shape
    new_T = min(MAX_T, int(T * scale))
    if new_T < 10: return data.copy()
...
def main():
    ...
    augmented_train = []
    # Only augment 'falling' (class 3)
    target_falling = 150
    falling_tracks = [arr for arr, lbl in zip(X_train, y_train) if lbl == 3]
    other_tracks = [(arr, lbl) for arr, lbl in zip(X_train, y_train) if lbl != 3]
    
    # Add other tracks as is
    augmented_train.extend(other_tracks)
    
    if falling_tracks:
        current_num = len(falling_tracks)
        multiplier = target_falling // current_num
        remainder = target_falling % current_num
        
        def get_aug_variant(idx, arr):
            if idx == 0: return arr
            elif idx == 1: return aug_flip(arr)
            elif idx == 2: return aug_noise(arr)
            elif idx == 3: return aug_temporal_scale(arr, 1.2)
            elif idx == 4: return aug_temporal_scale(arr, 0.8)
        ...
        for arr in falling_tracks:
            for variant_idx in range(multiplier):
                augmented_train.append((get_aug_variant(variant_idx, arr), 3))
                
        for track_idx in range(remainder):
            arr = falling_tracks[track_idx]
            augmented_train.append((get_aug_variant(multiplier, arr), 3))
    ...
    out_X_train = np.stack([pad_to_max_clamp(t[0]) for t in augmented_train])
    ...
    out_dir = Path(r"d:\Capstone2026\Action Predict\Labeled_data\new_data")
